...
io.on('connection', function (socket) {
  ...
  var sendFakeUpdates = function (n) {
    console.log('Sending fake updates');
    for (var i = 0; i < n; i++) {
      var msg = 'update #' + i;
      socket.emit('onTimelineUpdated', msg);
      console.log('Sent onTimelineUpdated', msg);
    }
  };
  ...
  if (_username) {
    ...
    socket.on('getUserList', function () {
      socket.emit('onGetUserListError', {
        code: 403,
        description: 'You must specify a username in your query'
      });
    });
  }
